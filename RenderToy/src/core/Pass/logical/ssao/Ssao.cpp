    ...
    int blurCount)
{
    ...
    // TODO: Register SsaoCB for FrameResource
    auto ssaoCBAddress = currFrame->GetResource<std::shared_ptr<UploadBuffer<SsaoPrePass::SsaoConstants>>>(
        "SsaoCB")->GetResource()->GetGPUVirtualAddress();
...
void Ssao::BuildResources()
{
    ...
    texDesc.Flags = D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET;  // ������Ⱦ
...
void Ssao::BuildRandomVectorTexture(ID3D12GraphicsCommandList* cmdList)
{
    ...
    for(int i = 0; i < 256; ++i)
    {
        for(int j = 0; j < 256; ++j)
        {
            ...
            XMFLOAT3 v(Math::MathHelper::RandF(), Math::MathHelper::RandF(), Math::MathHelper::RandF());
...
void Ssao::BuildOffsetVectors()
{
    ...
    for(int i = 0; i < 14; ++i)
	{
		...
		float s = Math::MathHelper::RandF(0.25f, 1.0f);
